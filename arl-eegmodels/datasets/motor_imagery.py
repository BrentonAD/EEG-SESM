...
class MotorImageryDataset:
    ...
    def load_data(self, n_files=None):
        ...
        self.X_train = tf.convert_to_tensor(data_train)
        self.y_train = tf.convert_to_tensor(label_train)
        self.X_val = tf.convert_to_tensor(data_val)
        self.y_val = tf.convert_to_tensor(label_val)
        if (data_test is not None) and (label_test is not None):
            self.X_test = tf.convert_to_tensor(data_test)
            self.y_test = tf.convert_to_tensor(label_test)
